...
hisseler=["AKENR","EUYO","KRVGD"]

url="https://www.isyatirim.com.tr/tr-tr/analiz/hisse/Sayfalar/sirket-karti.aspx?hisse=TUPRS"
r=requests.get(url)
...
for i in hisseler:
    ...
    if bilancosayı+1>=len(veri3.columns):
        veri3.to_excel("/Users/krtdnc/Desktop/Python/Python Projeler/İş Yatırım Toplu Bilanço Çekme/Bilancolar/{}.xlsx".format(hisse),index=False)

    else:
        veri3.drop(veri3.columns[-(len(veri3.columns)-bilancosayı+1):],axis=1,inplace=True)
        veri3.to_excel("/Users/krtdnc/Desktop/Python/Python Projeler/İş Yatırım Toplu Bilanço Çekme/Bilancolar/{}.xlsx".format(hisse),index=False)
